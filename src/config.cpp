...
std::ostream& operator<<(std::ostream& os, const Config& config)
{
    ...
    os << "  + output_filepath: " << config.outputDir << std::endl
       ...
       << "    - enable_bloom_threshed: " << config.features.extra.enableBloomThreshholded << std::endl
       << "    - enable_bloom_thresh_blurred: " << config.features.extra.enableBloomThreshBlurred << std::endl;

    

    os << "    - enable_multiple_rays_per_pixel: " << config.features.extra.enableMultipleRaysPerPixel << std::endl;
    os << "    - enable_multiple_rays_per_pixel_half_screen: " << config.features.extra.enableMultipleRaysPerPixelHalfScreen << std::endl;
...
Config readConfigFile(const std::filesystem::path& config_path)
{
    ...
    if (table["features"]["extra"]["enable_bloom_threshed"]) {
        config.features.extra.enableBloomThreshholded = table["features"]["extra"]["enable_bloom_threshed"]
                                                      .as_boolean()
                                                      ->value_or(false);
    }
    if (table["features"]["extra"]["enable_bloom_thresh_blurred"]) {
        config.features.extra.enableBloomThreshBlurred = table["features"]["extra"]["enable_bloom_thresh_blurred"]
                                                      .as_boolean()
                                                      ->value_or(false);
    }
    ...
    if (table["features"]["extra"]["enable_multiple_rays_per_pixel_half_screen"]) {
        config.features.extra.enableMultipleRaysPerPixelHalfScreen = table["features"]["extra"]["enable_multiple_rays_per_pixel_half_screen"].as_boolean()->value_or(false);
    }
